...
void Update_from_enrolled_students_file(STU_COURSE_NODE*& stu_course, std::string course_id, std::string filename, CR_NODE* course, STU_NODE* student) {
	...
	STU_NODE* cur_stu = student;


	while (cur_course->course.ID != course_id)
	...
	in.open(filename);
	out.open("test_student_course.csv", std::ofstream::app | std::ofstream::out);

	if (!in.is_open() || !out.is_open()) {
		my_print(5, 5, RED, "cannot open file");
		system("pause");
		return;
	}
	...
	while (!in.eof()) {
		...
		out << "\n" << i << "," << cur_course->course.Schoolyear << "," << cur_course->course.Semester << ",";
		std::getline(in, tmp, ',');
		std::getline(in, tmp, ',');
		out << tmp << ",";
		...
		while (cur_stu->student.StudentID != tmp)
			cur_stu = cur_stu->next;

		out << cur_stu->student.LName + cur_stu->student.FName << ",";
		new_stu_course->stu_course.StudentName = cur_stu->student.LName + " " + cur_stu->student.FName;

		out << cur_stu->student.Gender << ",";
		new_stu_course->stu_course.Gen = cur_stu->student.Gender;

		out << cur_stu->student.Classes.ClassID << ",";
		new_stu_course->stu_course.Class = cur_stu->student.Classes.ClassID;

		out << cur_course->course.ID << "," << cur_course->course.CName << "," << cur_course->course.Credits << "," << cur_course->course.Max_stdn << "," << cur_course->course.LNameTeacher + cur_course->course.FNameTeacher << ",";
		...
		out << cur_course->course.teacherID << "," << cur_course->course.dayOfWeek << "," << cur_course->course.session << "," << cur_course->course.startDate.day << "," << cur_course->course.startDate.month << "," << cur_course->course.endDate.day << "," << cur_course->course.endDate.month << ",";
		new_stu_course->stu_course.TeacherID = cur_course->course.teacherID;
		new_stu_course->stu_course.weekday = cur_course->course.dayOfWeek;
		new_stu_course->stu_course.session = cur_course->course.session;
		...
		out << new_stu_course->stu_course.midterm << "," << new_stu_course->stu_course.final << "," << new_stu_course->stu_course.other << "," << new_stu_course->stu_course.total;

		cur_stu_course->next = new_stu_course;
		new_stu_course->prev = nullptr;
		cur_stu_course = cur_stu_course->next;
		i++;
		cur_stu = student;
		std::getline(in, tmp, '\n');
	...
	in.close();
	out.close();
	my_print(5, 5, GREEN, "Update successfully");
	system("pause");
}